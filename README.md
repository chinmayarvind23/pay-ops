...
- A local operator CLI connects either model adapter and all six read tools with an expiring OS-account grant. A live control completed four valid local-model turns, real operational reads and an insufficient-evidence report, then recovered its completed journal unchanged. See [integration results](docs/operator-integration-results.md).
