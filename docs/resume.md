...
- Built an agentic incident-response system with LangChain/LangGraph that correlated Kubernetes health, logs, traces, deployment changes, and payment telemetry across **24 reproducible Kubernetes failure scenarios**, achieving **83.3% root-cause Recall@1** in predicate-assisted development replay.
...
- Added structured investigation traces, incident memory, deterministic approval gates, and operational telemetry, measuring **8.55-second median diagnostic replay latency** with **$0 provider charges** through local inference; deployed an interactive incident replay on **free Hugging Face hosting**.
